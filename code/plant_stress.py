...
class PlantStress(object):
    ...
    def _get_anomalies_array(self, ndvi_image, z1, z2):
    ...
    def segment_field(self, field, ndvi_path, start_date, end_date, request_id=10001):
        ...
        z2 = (
            pure_mu - z_score_anom * pure_sigma,
            pure_mu + z_score_anom * pure_sigma,
        )  # find anomalies in "noise-free" image

        img_anom = self._get_anomalies_array(ndvi_image, z1, z2)

        raster_path = ndvi_path.replace("_ndvi.tif", f"_field_{REQUEST_ID}.tif")
